...
export default function NavigationBar() {
  ...
  // neon shadow on scroll
  useMotionValueEvent(scrollY, "change", latest => {
    if (navRef.current) {
      const p = Math.min(latest / 500, 1);
      navRef.current.style.setProperty(
        "--depth-shadow",
        `0 10px ${30+p*20}px ${-10-p*10}px rgba(0,0,0,${0.2+p*0.1}), 0 0 ${2+p*10}px rgba(255,243,24,${0.1+p*0.1})`
      );
    }
  });
  ...
  return (
    <>
      <motion.nav
        ...
        style={{ boxShadow: "var(--depth-shadow)" }}
        className="fixed top-0 left-0 right-0 z-50 h-20 sm:h-22 md:h-24 bg-black/60 backdrop-blur-md px-4 sm:px-6 flex items-center"
      >
        <div className="container mx-auto flex items-center justify-between">
          {/* Logo with glow and text image */}
          <div className="flex items-center">
            ...
            >
              <motion.img
                ...
                animate={{ 
                  filter: [
                    'drop-shadow(0 0 8px rgba(255, 243, 24, 0.5))', 
                    'drop-shadow(0 0 12px rgba(255, 243, 24, 0.7))', 
                    'drop-shadow(0 0 8px rgba(255, 243, 24, 0.5))'
                  ]
                }}
                transition={{
                ...
                style={{
                  filter: 'drop-shadow(0 0 8px rgba(255, 243, 24, 0.5))'
                }}
              />
            </a>
            
            {/* Text image beside logo - adjusted vertical alignment */}
            <a
              href="#hero"
              onClick={e => { e.preventDefault(); scrollToSection('hero'); }}
            ...
            {menuItems.map((item) => (
              ...
              >
                ...
                <item.icon className={cn(
                  ...
                  active === item.id ? "text-gym-yellow" : "text-white/70"
                )} />
                <span className={cn(
                  "text-[10px] transition-colors",
                  active === item.id ? "text-gym-yellow font-medium" : "text-white/70"
                )}>
                  {item.label}
                </span>
              </motion.button>
